...
public class GameSettingsTransformer implements PatcherTransformer {
  ...
  public void transform(ClassNode classNode, String name) {
    MethodNode onGuiClosedMethod =
        new MethodNode(Opcodes.ACC_PUBLIC, "onGuiClosed", "()V", null, null);
    onGuiClosedMethod.instructions.add(onGuiClosed());
    classNode.methods.add(onGuiClosedMethod);

    FieldNode needsResourceRefreshField =
        new FieldNode(Opcodes.ACC_PRIVATE, "needsResourceRefresh", "Z", null, null);
      ...
      if (methodName.equals("setOptionFloatValue") || methodName.equals("func_74304_a")) {
        ...
        while (iterator.hasNext()) {
          ...
          if (node.getOpcode() == Opcodes.INVOKEVIRTUAL) {
            ...
            if (methodInsnName.equals("scheduleResourcesRefresh")
                || methodInsnName.equals("func_175603_A")) {
              methodNode.instructions.insertBefore(
                  node.getPrevious().getPrevious(), insertBoolean());
            }
          }
        }
