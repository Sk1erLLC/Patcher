...
public class CloudRenderer implements IResourceManagerReloadListener {
    ...
    public void checkSettings() {
        boolean newEnabled = PatcherConfig.gpuCloudRenderer && mc.gameSettings.shouldRenderClouds() != 0 && mc.theWorld != null && mc.theWorld.provider.isSurfaceWorld();

        if (isBuilt() && (!newEnabled || mc.gameSettings.shouldRenderClouds() != cloudMode || mc.gameSettings.renderDistanceChunks != renderDistance)) {
            dispose();
        }
