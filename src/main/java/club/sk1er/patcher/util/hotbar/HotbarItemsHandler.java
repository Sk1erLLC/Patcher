...
public class HotbarItemsHandler {
  ...
  public void renderItemCount(final RenderGameOverlayEvent.Post e) {
    ...
    if (mc.thePlayer.getCurrentEquippedItem() != null) {
      ...
      if (count > 1 || (holdingBow && count > 0)) {
        int offset =
            mc.playerController.getCurrentGameType() == WorldSettings.GameType.CREATIVE ? 10 : 0;
        ScaledResolution resolution = e.resolution;
        mc.fontRendererObj.drawString(
            String.valueOf(count),
            resolution.getScaledWidth() - mc.fontRendererObj.getStringWidth(String.valueOf(count))
                >> 1,
  ...
  public void renderEnchantments(final RenderGameOverlayEvent.Post e) {
    ...
    if (heldItemStack != null) {
      String toDraw =
          heldItemStack.getItem() instanceof ItemPotion
              ? getPotionEffectString(heldItemStack)
              : getEnchantmentString(heldItemStack);
